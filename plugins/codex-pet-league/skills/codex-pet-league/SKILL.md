...
Use this skill when the user asks about their Codex pet, hatch asset upload, daily XP, Training Reports, matchmaking, invite codes, turn battles, replays, leaderboard, or anti-cheat/admin status.
...
3. `pet_create` with `atlas_path` to upload a Codex hatch atlas PNG and register an official pet.
4. `training_report_draft`, then `training_report_submit` after user approval.
5. `matchmaking_join` for random ranked or casual queue.
6. `friend_invite_create` and `friend_invite_accept` for invite-code battles.
7. `battle_get`, `battle_action_options`, then `battle_action` for active turns.
...
npm run cli -- next
npm run cli -- daily
npm run cli -- pet create --name Pebble --primary Forge --secondary Trace --atlas C:\path\hatch.png
