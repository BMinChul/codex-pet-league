...
- Current web companion shell uses a desktop-first League client direction: top navigation, player-first Home/Battle/Training/Leaderboard/Replays/Profile tabs, a contextual Get Started import tab before the first pet, an admin-only Admin tab, a top-right Sign In modal, and a compact identity/rank chip after login. Keep future UI work closer to competitive game clients and esports/profile dashboards than to generic marketing pages.
