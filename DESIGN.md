# Codex Pet League Design.md

Source: `VoltAgent/awesome-design-md`, `design-md/voltagent/DESIGN.md`.

Use this as the installed visual reference for Codex Pet League's web companion UI: dark, terminal-native, developer-focused, and high-contrast with a restrained emerald accent.

Project overrides:

- Codex Pet League remains Codex App and CLI-first. The web UI is a companion for sign-in, status, profiles, battles, replays, leaderboards, and ops.
- Do not present browser-side pet creation as the primary official flow. Official pet registration happens from a local OpenAI `hatch-pet` package through CLI/MCP import.
- Keep UI compact, scannable, and operations-oriented; avoid marketing-page hero composition for the main app surface.
- Follow `AGENTS.md` and system UI constraints when they differ from this imported reference.
